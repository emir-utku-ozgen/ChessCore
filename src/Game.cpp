...
namespace chess{
    ...
    if (piece == nullptr) {
        throw IllegalMoveException("Baslangic karesinde tas yok.");
    }

    if (piece->getColor() != m_activeColor) {
        throw IllegalMoveException("Sira bu renkte degil.");
    }
    ...
    if (it != validMoves.end()) {
        ...
        if (selfCheck) {
            ...
            throw IllegalMoveException("Bu hamle kendi kralinizi sahta birakir.");
        } else {
    ...
    throw IllegalMoveException("Bu hamle, tasin hareket kurallarina uymuyor.");
}
